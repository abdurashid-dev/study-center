<div>
    ...
    <x-jet-input id="full_name" type="text" name="full_name" :value="$student->full_name" required
                 autofocus/>
    ...
    <select
        ...
        @foreach($groups as $group)
            <option value="{{$group->id}}"
                    @foreach($student->groups as $studentGroup)
                        @if($group->id == $studentGroup->group_id)
                            selected
                @endif
                @endforeach
            >{{$group->name}}</option>
...
<div>
    ...
    <x-jet-input id="address" type="text" name="address" :value="$student->address" autofocus/>
    <x-jet-input-error for="address" class="mt-2"/>
</div>
<div>
    <x-jet-label for="description" value="Tavsif"/>
    <x-textarea id="description" name="description">{!!$student->description!!}</x-textarea>
